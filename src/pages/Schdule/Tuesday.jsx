...
function Tuesday() {
  const tuesday = useSelector((state) => state.schedule.schedule).find(
    (sch) => sch.day === "Tuesday"
  );

  return (
    <>
      {tuesday.classes.map((tue) => (
        <div className="flex justify-around bg-[#f2f2f2] py-8 w-[95%] max-md:flex-col items-center text-center max-md:gap-5">
          <div className="lg:w-[25%]">
            ...
            <h2 className="text-xl">{tue.class_name}</h2>
          </div>
          <div className="lg:w-[25%]">
            <p className="text-[#646464] text-sm my-2">Time</p>
            <h2 className="text-lg">{tue.class_time}</h2>
          </div>
          <div className="lg:w-[25%]">
            <p className="text-[#646464] text-sm my-2">Trainer</p>
            <h2 className="text-xl">{tue.class_trainer}</h2>
          </div>
